...
func receiveRoutine(ws *websocket.Conn) error {
    for {
        ...
        // dispatch from type
        switch msg["type"] {
        case "hello": fmt.Println("Connected!")
        case "bot_added": onBotAdded(msg)
        case "channel_created": onChannelCreated(msg)
        case "channel_joined": onChannelJoined(msg)
        case "group_joined": onGroupJoined(msg)
        case "message": onMessage(msg)
        case "team_join": onTeamJoin(msg)
        case "user_change": onUserChange(msg)
        }
    }
...
func onMessage(msg map[string]interface{}) {
    switch msg["subtype"] {
    case nil: onPureMessage(msg)
    case "file_mention": return
    case "file_share": onMessageFileShare(msg)
    case "me_message": onMessageMe(msg)
    case "message_changed": onMessageChanged(msg)
    case "message_replied": return
    }
}
